...
async def web_search_wrapper(query: str, search_context_size: str = "medium"):
    """Wrapper for web search using web_search_tool_v2"""
    try:
        # Note: In actual implementation, you'll need to call web_search_tool_v2
        # For now, this is a simplified version
        # The actual call would be through the tool system
        from backend.channel.search_integration import perform_web_search
        results = await perform_web_search(query, search_context_size)
        return results
    except Exception as e:
        logger.error(f"Web search error: {e}")
        return f"Новости по теме '{query}' (поиск временно недоступен)"
...
async def create_and_post(fixed_topic: str = None):
    ...
    try:
        ...
        news_fetcher = NewsFetcher(web_search_wrapper)
